...
class car_info:
	def __init__(self, count=2):
		self.wheel_count = count #default
		self.wheel_radius = [0]*count
		self.wheel_density = [0]*count
		self.wheel_vertex = [0]*count
...
#		super(do_stuff,self).__init__() #uncoment for framework stuff
			...
			for index,cars in enumerate(self.population_data):
				if not cars.isDead:
					...
					if cars.isDead:
						...
						#if you want to keep all the cars on the screen, (only for testing) comment out the bottom 5 lines
	...
	def mate2ptCrossover(self,parents):
		#Note: This function has been modified from the original to better support my functions
		...
		# Set the swap points
		total_attributes = self.getTotalAttr(parents[0])
		attribute_index = 0
		...
		# Create new car definition and set the wheel count
		curr_parent = 0
		child = car_info(parents[curr_parent].get_wheel_count()) #also sets the wheel count
		attribute_index += 1
		curr_parent = self.which_parent(attribute_index,curr_parent,swap_point1,swap_point2)
		
		# Wheel Radius
		for i in range(child.get_wheel_count()):
			usedParent = curr_parent
			if (i >= len(child.wheel_radius) or i >= len(parents[usedParent].wheel_radius)):
				usedParent = 1 if curr_parent == 0 else 0
			child.wheel_radius[i] = parents[usedParent].wheel_radius[i]
			attribute_index += 1
			curr_parent = self.which_parent(attribute_index,curr_parent,swap_point1,swap_point2)
		
		# Wheel vertex
		for i in range(child.get_wheel_count()):
			if (i >= len(child.wheel_vertex) or i >= len(parents[usedParent].wheel_vertex)):
				usedParent = 1 if curr_parent == 0 else 0
			child.wheel_vertex[i] = parents[usedParent].wheel_vertex[i]
			attribute_index += 1
			curr_parent = self.which_parent(attribute_index,curr_parent,swap_point1,swap_point2)
		
		# Wheel Density
		for i in range(child.get_wheel_count()):
			if (i >= len(child.wheel_density) or i >= len(parents[usedParent].wheel_density)):
				usedParent = 1 if curr_parent == 0 else 0
			child.wheel_density[i] = parents[usedParent].wheel_density[i]
			attribute_index += 1
			curr_parent = self.which_parent(attribute_index,curr_parent,swap_point1,swap_point2)

		# Vertices
		for i in range(len(child.get_vertex_list())):
			if (i >= len(child.vertex_list) or i >= len(parents[usedParent].vertex_list)):
				usedParent = 1 if curr_parent == 0 else 0
			child.vertex_list[i] = parents[usedParent].vertex_list[i]
			attribute_index += 1
			curr_parent = self.which_parent(attribute_index,curr_parent,swap_point1,swap_point2)
	...
	def mate1ptCrossover(self,parents):
		#for 1 swapPoint
		#if we have 10 attributes and the swappoint is 5
		#the child will have the following attribute array:
		#child = parent1[1..swapPoint] + parent2[swapPoint+1 ...10]
		...
		# Create new car definition and set the wheel count
		curr_parent = 0
		child = car_info(parents[curr_parent].get_wheel_count())
		child.set_wheel_count(parents[curr_parent].get_wheel_count())
		attribute_index += 1
		curr_parent = self.which_parent(attribute_index,curr_parent,swap_point,swap_point)
	...
	def mateUniformCrossover(self,parents):
		#for each attribute, a coin is tossed to decide which parent the child will inherit from
	...
	# Wrapper for coin toss function
	...
	def mutate(self,inchild):
		child = car_info(inchild.get_wheel_count())
		
		# wheel_count
		child.set_wheel_count = inchild.get_wheel_count()
		initial = child.wheel_count
		if (self.shouldMutate(child)):
			if (self.coinFlip() and child.wheel_count < 3):
				child.wheel_count += 1
			else:
				child.wheel_count = child.wheel_count - 1 if child.wheel_count > 1 else child.wheel_count

			# Add a random wheel
			if child.wheel_count > initial:
				index_left = [i for i in range(8)]
				vertex = None
				while (vertex in child.wheel_vertex or vertex == None):
					vertex = index_left[int(random.random() * (len(index_left)-1))]
				# print("Mutation: Added a wheel!")
				child.wheel_radius.append(random.random()*wheelMaxRadius+wheelMinRadius)
				child.wheel_density.append(random.random()*wheelMaxDensity+wheelMinDensity)
				child.wheel_vertex.append(vertex)
			# Remove a wheel
			elif child.wheel_count < initial:
				# print("Mutation: Removed a wheel!")
				wheel = random.randint(1,child.get_wheel_count())
				child.wheel_radius.pop(wheel)
				child.wheel_density.pop(wheel)
				child.wheel_vertex.pop(wheel)

		# wheel radius
		for i in range(child.get_wheel_count()):
			if (i < len(inchild.wheel_radius)):
				child.wheel_radius[i] = inchild.wheel_radius[i]
				if (self.shouldMutate(child)):
					# print("Mutated " + "Wheel Radius!")
					child.wheel_radius[i] = random.random()*wheelMaxRadius+wheelMinRadius
		
		# wheel vertex
		index_left = [i for i in range(8)]
		for i in range(child.get_wheel_count()):
			if (i < len(inchild.wheel_vertex)):
				child.wheel_vertex[i] = inchild.wheel_vertex[i]
				if (self.shouldMutate(child)):
					# print("Mutated " + "Wheel Vertex!")
					index_of_next = int(random.random() * (len(index_left)-1))
					child.wheel_vertex[i] = index_left[index_of_next]
		
		# wheel density
		for i in range(child.get_wheel_count()):
			if (i < len(inchild.wheel_density)):
				child.wheel_density[i] = inchild.wheel_density[i]
				if (self.shouldMutate(child)):
					# print("Mutated " + "Wheel Density!")
					child.wheel_density_values = random.random()*wheelMaxDensity+wheelMinDensity
		
		# vertex list
		for i in range(len(child.get_vertex_list())):
			if (i < len(inchild.vertex_list)):
				child.vertex_list[i] = inchild.vertex_list[i]
				if (self.shouldMutate(child)):
					# print("Mutated " + "Vertex!")
					self.newVertexVal(child, i)
		
		# chassis density
		child.chassis_density = inchild.chassis_density
		if (self.shouldMutate(child)):
			# print("Mutated " + "Chassis Density!")
			child.chassis_density = random.random()*chassisMaxDensity+chassisMinDensity
	...
	def newVertexVal(self, child, i):
		if i == 0:
			child.vertex_list[i] = (b2Vec2(random.random()*chassisMaxAxis + chassisMinAxis,0))
		elif i == 1:
			child.vertex_list[i] = (b2Vec2(random.random()*chassisMaxAxis + chassisMinAxis,random.random()*chassisMaxAxis + chassisMinAxis))
		elif i == 2:
			child.vertex_list[i] = (b2Vec2(0,random.random()*chassisMaxAxis + chassisMinAxis))
		elif i == 3:
			child.vertex_list[i] = (b2Vec2(-random.random()*chassisMaxAxis - chassisMinAxis,random.random()*chassisMaxAxis + chassisMinAxis))
		elif i == 4:
			child.vertex_list[i] = (b2Vec2(-random.random()*chassisMaxAxis - chassisMinAxis,0))
	...
	def getTotalAttr(self, child):
		totalAttr = 2 # wheelcount + chassisdensity
		totalAttr += len(child.wheel_radius)
		totalAttr += len(child.wheel_vertex)
		totalAttr += len(child.wheel_density)
		totalAttr += len(child.vertex_list)
		return totalAttr

	def shouldMutate(self, child):
		totalAttr = self.getTotalAttr(child)
		mutationRate = int((1/totalAttr)*100)
		mutationValue = random.randint(1,101)
		return True if (mutationValue <= mutationRate) else False
	...
	def next_generation(self):
		...
		while len(new_population) < self.population_size:
			...
			# child_def = self.mate2ptCrossover(parents) #we're passing only the indices of the parents
			# child_def = self.mate1ptCrossover(parents) # COMMENT OR UNCOMMENT HERE TO RUN THE RIGHT CROSSOVER ROUTINE
			child_def = self.mateUniformCrossover(parents)
			child_def = self.mutate(child_def) # UNCOMMENT THIS WHEN YOU'VE IMPLEMENTED MUTATION

			child = car(self.world,random = False,car_def = child_def)
